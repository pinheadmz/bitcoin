...
public:
    ...
    // Checked during DisconnectClients(). These operations take place in the Sockman I/O loop,
    // however this might also be set by a worker thread during an "optimistic send".
    ...
    // Normally set and read inside the Sockman I/O thread,
    // but may also be set by a worker thread during an "optimistic send".
