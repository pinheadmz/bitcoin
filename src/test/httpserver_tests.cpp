...
BOOST_AUTO_TEST_CASE(test_query_parameters)
{
    ...
    BOOST_CHECK(!GetQueryParameterFromUri(uri, "p1").has_value());

    // Single parameter
    uri = "localhost:8080/rest/endpoint/someresource.json?p1=v1";
    BOOST_CHECK_EQUAL(GetQueryParameterFromUri(uri, "p1").value(), "v1");
    BOOST_CHECK(!GetQueryParameterFromUri(uri, "p2").has_value());

    // Multiple parameters
    uri = "/rest/endpoint/someresource.json?p1=v1&p2=v2";
    BOOST_CHECK_EQUAL(GetQueryParameterFromUri(uri, "p1").value(), "v1");
    BOOST_CHECK_EQUAL(GetQueryParameterFromUri(uri, "p2").value(), "v2");
    ...
    uri = "/rest/endpoint/someresource.json&p1=v1&p2=v2%";
    BOOST_CHECK_EXCEPTION(GetQueryParameterFromUri(uri, "p1"), std::runtime_error, HasReason("URI parsing failed, it likely contained RFC 3986 invalid characters"));
}
