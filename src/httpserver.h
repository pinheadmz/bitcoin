...
public:
     ...
     */
    size_t GetConnectionsCount() { return m_connected.size(); }
...
private:
    ...
    /*
     * Set in the I/O thread and only checked in the main thread when
     * shutting down, to wait for all clients to be disconnected.
     */
     ...
     */
    void MaybeDispatchRequestsFromClient(std::shared_ptr<HTTPClient> client);
