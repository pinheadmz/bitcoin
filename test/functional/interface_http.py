...
class HTTPBasicsTest (BitcoinTestFramework):
    ...
    def check_connection_limit(self):
        ...
        with self.node.assert_debug_log(
            ...
            unexpected_msgs = ["Cannot accept new connection"]
        ):
        ...
        with self.node.assert_debug_log(
            expected_msgs = [f"Cannot accept new connection: would exceed limit of {MAX_HTTP_CONNECTIONS}"],
            unexpected_msgs = ["method=never_accepted"]
        ):
            conn = BitcoinHTTPConnection(self.node)
            # Only try for a second. The default client timeout is so long that the
            # original 128 connections will start to hit the default rpcservertimeout=30
            # and get disconnected, opening new connection slots on the server.
            # Then this connection would eventually unexpectedly succeed.
            conn.set_timeout(1)
            try:
