...
{
    return std::erase_if(m_connected,
                         [&](auto& client){ return client == http_client; }) > 0;
}
...
void HTTPServer::NewSockAccepted(std::unique_ptr<Sock>&& sock, const CService& me, const CService& them)
{
...
HTTPServer::IOReadiness HTTPServer::GenerateWaitSockets()
{
...
void HTTPServer::MaybeDispatchRequestsFromClient(std::shared_ptr<HTTPClient> client)
{
